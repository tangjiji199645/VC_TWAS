...
gwas_SS_path = "/sample_data/simulation/model2_GWAS.txt"
...
###load true effect sizes of causal SNPs
true_beta = pd.read_csv(true_beta_path, sep = "\t")


true_beta_value = true_beta.sort_values(by = "POS").BETA
true_beta_doage = test_dosage.loc[test_dosage.loc[:,1].isin(true_beta.rsID),5:].T
beta_sim = np.zeros(len(true_beta))

for i in range(len(true_beta)):
    beta_sim[i] = np.random.normal(0, np.abs(true_beta_value[i]), 1)
    
Y = true_beta_doage.dot(beta_sim)
var_Y = np.var(Y)
gamma = np.sqrt(pheno_h2 / var_Y)
error = np.random.normal(0, stdev, len(Y))
exp_simulated = gamma * Y + error
...
SNP_info.to_csv(gwas_SS_path, index=None, sep='\t', mode='w')